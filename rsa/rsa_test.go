package rsa

import (
	"testing"

	"github.com/stretchr/testify/assert"
)

var (
	msg              = "床前明月光，疑是地上霜，举头望明月，低头思故乡"
	base64PubKey1024 = "MIGJAoGBAL8Om9dxnaqfKarF4bewdCTXmymXt4K5FBf3FPT413bq/3zoAaHTWUfzNBmrcAW+rYr5tXDPD+WncqKqI9yo+zW+w+mgOOkSbQsGUDQuah5B4tRNBgV533w+ljSGM+7PxgOhapFyh2vwD/BGQ4GafrZeU2hyHXQDgfL95dfrsbI/AgMBAAE="
	base64PriKey1024 = ""
	base64PubKey2048 = ""
	base64PriKey2048 = ""

	hexPubKey1024 = ""
	hexPriKey1024 = ""
	hexPubKey2048 = ""
	hexPriKey2048 = ""
)

func TestRsaEncryptBase64(t *testing.T) {
	base64Key1024, err := GenerateRsaKeyBase64(1024)
	assert.Nil(t, err)
	base64Key2048, err := GenerateRsaKeyBase64(2048)
	assert.Nil(t, err)
	_, err = GenerateRsaKeyBase64(204811)
	assert.NotNil(t, err)

	// good
	base64CipherText, err := EncryptToBase64([]byte(msg), base64Key1024.PublicKey)
	assert.Nil(t, err)
	// bad
	_, err = EncryptToBase64([]byte(msg), "badkey")
	assert.NotNil(t, err)
	_, err = EncryptToBase64([]byte(msg), hexPubKey1024)
	assert.NotNil(t, err)
	// good
	plainText, err := DecryptByBase64(base64CipherText, base64Key1024.PrivateKey)
	assert.Nil(t, err)
	assert.Equal(t, string(plainText), msg)
	// bad priKey
	_, err = DecryptByBase64(base64CipherText, "badPriKey")
	assert.NotNil(t, err)
	_, err = DecryptByBase64(base64CipherText, base64Key2048.PublicKey)
	assert.NotNil(t, err)
	_, err = DecryptByBase64(base64CipherText, base64Key2048.PrivateKey)
	assert.NotNil(t, err)
	_, err = DecryptByBase64(base64CipherText, hexPriKey1024)
	assert.NotNil(t, err)
	_, err = DecryptByBase64(base64CipherText, hexPriKey2048)
	assert.NotNil(t, err)
	_, err = DecryptByBase64(base64CipherText, hexPubKey1024)
	assert.NotNil(t, err)
	_, err = DecryptByBase64("badtext", base64Key1024.PrivateKey)
	assert.NotNil(t, err)

	// good
	base64CipherText, err = EncryptToBase64([]byte(msg), base64Key2048.PublicKey)
	assert.Nil(t, err)
	plainText, err = DecryptByBase64(base64CipherText, base64Key2048.PrivateKey)
	assert.Nil(t, err)
	assert.Equal(t, string(plainText), msg)

	// good
	base64CipherText, err = EncryptToBase64([]byte(msg), base64PubKey1024)
	assert.Nil(t, err)
	plainText, err = DecryptByBase64(base64CipherText, base64PriKey1024)
	assert.Nil(t, err)
	assert.Equal(t, string(plainText), msg)

	// good
	base64CipherText, err = EncryptToBase64([]byte(msg), base64PubKey2048)
	assert.Nil(t, err)
	plainText, err = DecryptByBase64(base64CipherText, base64PriKey2048)
	assert.Nil(t, err)
	assert.Equal(t, string(plainText), msg)
}

func TestRsaEncryptHex(t *testing.T) {
	hexKey1024, err := GenerateRsaKeyHex(1024)
	assert.Nil(t, err)
	hexKey2048, err := GenerateRsaKeyHex(2048)
	assert.Nil(t, err)
	_, err = GenerateRsaKeyHex(2048111)
	assert.NotNil(t, err)

	// good
	hexCipherText, err := EncryptToHex([]byte(msg), hexKey1024.PublicKey)
	assert.Nil(t, err)
	// bad
	_, err = EncryptToHex([]byte(msg), "badkey")
	assert.NotNil(t, err)
	// good
	plainText, err := DecryptByHex(hexCipherText, hexKey1024.PrivateKey)
	assert.Nil(t, err)
	assert.Equal(t, string(plainText), msg)
	// bad
	_, err = DecryptByHex(hexCipherText, "badkey")
	assert.NotNil(t, err)
	// bad priKey
	_, err = DecryptByHex(hexCipherText, hexPriKey2048)
	assert.NotNil(t, err)
	_, err = DecryptByHex(hexCipherText, hexPriKey1024)
	assert.NotNil(t, err)
	_, err = DecryptByHex(hexCipherText, base64PriKey2048)
	assert.NotNil(t, err)
	_, err = DecryptByHex(hexCipherText, base64PriKey1024)
	assert.NotNil(t, err)
	_, err = DecryptByHex("ssss", hexKey1024.PrivateKey)
	assert.NotNil(t, err)

	// good
	hexCipherText, err = EncryptToHex([]byte(msg), hexKey2048.PublicKey)
	assert.Nil(t, err)
	plainText, err = DecryptByHex(hexCipherText, hexKey2048.PrivateKey)
	assert.Nil(t, err)
	assert.Equal(t, string(plainText), msg)

	// good
	hexCipherText, err = EncryptToHex([]byte(msg), hexPubKey1024)
	assert.Nil(t, err)
	plainText, err = DecryptByHex(hexCipherText, hexPriKey1024)
	assert.Nil(t, err)
	assert.Equal(t, string(plainText), msg)

	// good
	hexCipherText, err = EncryptToHex([]byte(msg), hexPubKey2048)
	assert.Nil(t, err)
	plainText, err = DecryptByHex(hexCipherText, hexPriKey2048)
	assert.Nil(t, err)
	assert.Equal(t, string(plainText), msg)
}

func TestRsaSignBase64(t *testing.T) {
	base64Key1024, err := GenerateRsaKeyBase64(1024)
	assert.Nil(t, err)
	base64Key2048, err := GenerateRsaKeyBase64(2048)
	assert.Nil(t, err)
	_, err = GenerateRsaKeyBase64(204811)
	assert.NotNil(t, err)

	base64Sign1024, err := SignBase64([]byte(msg), base64Key1024.PrivateKey)
	assert.Nil(t, err)
	_, err = SignBase64([]byte(msg), hexPriKey2048)
	assert.NotNil(t, err)
	res := VerifySignBase64([]byte(msg), base64Sign1024, base64Key1024.PublicKey)
	assert.Equal(t, res, true)
	res = VerifySignBase64([]byte(msg), base64Sign1024, base64Key2048.PublicKey)
	assert.Equal(t, res, false)
	res = VerifySignBase64([]byte(msg), "11111", "badpubkey")
	assert.Equal(t, res, false)
	res = VerifySignBase64([]byte(msg), "11111", base64Key1024.PublicKey)
	assert.Equal(t, res, false)
	res = VerifySignBase64([]byte(msg), base64Sign1024, base64Key2048.PublicKey)
	assert.Equal(t, res, false)

	base64Sign2048, err := SignBase64([]byte(msg), base64Key2048.PrivateKey)
	assert.Nil(t, err)
	res = VerifySignBase64([]byte(msg), base64Sign2048, base64Key2048.PublicKey)
	assert.Equal(t, res, true)
	res = VerifySignBase64([]byte(msg), base64Sign2048, base64Key1024.PublicKey)
	assert.Equal(t, res, false)
	res = VerifySignBase64([]byte(msg), "11111", "badpubkey")
	assert.Equal(t, res, false)
	res = VerifySignBase64([]byte(msg), base64Sign2048, base64Key1024.PublicKey)
	assert.Equal(t, res, false)
	res = VerifySignBase64([]byte(msg), base64Sign2048, base64Key1024.PrivateKey)
	assert.Equal(t, res, false)
	res = VerifySignBase64([]byte(msg), base64Sign2048, base64Key2048.PrivateKey)
	assert.Equal(t, res, false)
}

func TestRsaSignHex(t *testing.T) {
	hexKey1024, err := GenerateRsaKeyHex(1024)
	assert.Nil(t, err)
	hexKey2048, err := GenerateRsaKeyHex(2048)
	assert.Nil(t, err)
	_, err = GenerateRsaKeyHex(2048111)
	assert.NotNil(t, err)

	hexSign1024, err := SignHex([]byte(msg), hexKey1024.PrivateKey)
	assert.Nil(t, err)
	_, err = SignHex([]byte(msg), hexKey1024.PublicKey)
	assert.NotNil(t, err)
	res := VerifySignHex([]byte(msg), hexSign1024, hexKey1024.PublicKey)
	assert.Equal(t, res, true)
	res = VerifySignHex([]byte(msg), hexSign1024, hexKey2048.PublicKey)
	assert.Equal(t, res, false)
	res = VerifySignHex([]byte(msg), "11111", "badpubkey")
	assert.Equal(t, res, false)
	res = VerifySignHex([]byte(msg), "282010100db", hexKey1024.PublicKey)
	assert.Equal(t, res, false)
	res = VerifySignHex([]byte(msg), hexSign1024, hexKey2048.PublicKey)
	assert.Equal(t, res, false)

	hexSign2048, err := SignHex([]byte(msg), hexKey2048.PrivateKey)
	assert.Nil(t, err)
	_, err = SignHex([]byte(msg), hexPubKey2048)
	assert.NotNil(t, err)
	res = VerifySignHex([]byte(msg), hexSign2048, hexKey2048.PublicKey)
	assert.Equal(t, res, true)
	res = VerifySignHex([]byte(msg), hexSign2048, hexKey1024.PublicKey)
	assert.Equal(t, res, false)
	res = VerifySignHex([]byte(msg), "0a0282010100d", "badpubkey")
	assert.Equal(t, res, false)
	res = VerifySignHex([]byte(msg), "0a0282010100d", hexKey1024.PublicKey)
	assert.Equal(t, res, false)
	res = VerifySignHex([]byte(msg), "xxxx", "badpubkey")
	assert.Equal(t, res, false)
	res = VerifySignHex([]byte(msg), hexSign2048, hexKey1024.PublicKey)
	assert.Equal(t, res, false)
	res = VerifySignHex([]byte(msg), hexSign2048, hexKey1024.PrivateKey)
	assert.Equal(t, res, false)
	res = VerifySignHex([]byte(msg), hexSign2048, hexKey2048.PrivateKey)
	assert.Equal(t, res, false)
}
